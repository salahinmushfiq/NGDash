...
const PatternAPI = () => {
  ...
  <div role="status">
    ...
    //   axios.get('https://dummyjson.com/users/').then((response)=>{
          ...
          await axios.post('http://127.0.0.1:8000/classification/pattern/kms/data_cleaning/',initialDataJson, customConfig)
        .then(async response =>{
          ...
          await axios.post('http://127.0.0.1:8000/classification/data_pre_processing/',cleanedDataJson, customConfig)
        .then(async response =>{
          ...
          setPreProcessedData(preProcessedDataResponse);
           ...
           await axios.post('http://127.0.0.1:8000/classification/data_scaling/',preProcessedDataResponseJson, customConfig) .then(
            async response =>{
            ...
            await axios.post('http://127.0.0.1:8000/classification/modeled_data_hierarchical/',scaledDataResponseJson, customConfig) .then(
              async response =>{
                ...
                setHierarchicalFrequencyByClusterIdBoxPlotDataValues(frequencyBoxPlotData);
                setHierarchicalRecencyByClusterIdBoxPlotDataValues(recencyBoxPlotData);
...
let sillhouette_scores_data_columns=[
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-xl text-black dark:text-gray-200 dark:bg-secondary-dark-bg border-white">
      <Header category="Page" title="Pattern API"/>
      ...
      <div style={{ height: 380}} className="mt-8 min-w-fit px-4">
            {hierarchicalModeledData.length > 0 ? 
            <DataGrid
                
                sx={sx}
                initialState={initialState}
                columns={modeled_data_column}
                rows={hierarchicalModeledData}
                getRowId={(row) => row.Amount+row.Recency}
                rowsPerPageOptions={[5,10,20]}
                pageSize={pageSizeTable4}
                onPageSizeChange={(newPageSize) => setPageSizeTable4(newPageSize)}
